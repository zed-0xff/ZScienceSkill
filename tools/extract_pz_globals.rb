...
unless Dir.exist?(PZ_LUA_PATH)
  puts "❌ PZ Lua path not found: #{PZ_LUA_PATH}"
  puts "Update PZ_LUA_PATH in this script to match your installation"
  exit 1
end

puts "📂 Scanning PZ Lua files in: #{PZ_LUA_PATH}"
...
puts "\n📋 Found #{globals.size} total globals (#{MANUAL_GLOBALS.size} manual + #{globals.size - MANUAL_GLOBALS.size} extracted)"
...
if ARGV.include?("--write")
  ...
  puts "✅ Wrote #{globals.size} globals to: #{output_file}"
  puts "   (#{MANUAL_GLOBALS.size} manual + #{extracted.size} auto-extracted)"
else
  puts "\n" + output.join("\n")
  puts "\n💡 To write directly to .luacheckrc_globals.lua:"
